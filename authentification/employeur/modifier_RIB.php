...
<div class="w3-main" style="margin-left:40px;margin-right:40px">
  ...
  <!-- Header / Solde -->
    <div class="w3-container" style="margin-top:20px" id="showcase">
        <h1 class="w3-xxxlarge w3-text-red"><b>Modifier votre RIB</b></h1>
        <hr style="width:50px;border:5px solid red" class="w3-round">
    </div>

  <!-- Menu modification -->

<p>Votre RIB actuelle : <?php echo $_SESSION['RIB'] ?></p><br />

<form method="post" class="w3-container w3-card-4 w3-light-grey">
  <p>      
  <label>Veuillez saisir votre nouveau RIB : </label>
...
<?php
    if(isset($_POST['formsend'])){
        ...
        if(!empty($rib)){
            ...
            $requete1->execute();
            $id=$requete1->fetch();
            $requete2 = $db->prepare("UPDATE employeur SET RIB=:rib WHERE idEmployeur=:id");
            $requete2->bindParam(':id', $id[0], PDO::PARAM_INT);
            $requete2->bindParam(':rib', $rib, PDO::PARAM_STR);
            $requete2->execute();
